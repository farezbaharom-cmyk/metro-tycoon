...
function renderSide(){
  ...
  t.classList.toggle('turn-active',S.phase!=='over');
  ...
  document.getElementById('players').innerHTML=S.players.map((q,k)=>{
    ...
    return `<li class="pl ${k===S.turn&&S.phase!=='over'?'cur':''} ${q.bankrupt?'out':''}" style="--player-color:${q.color}">${trainMark(k,q.color)}<span class="nmx">${NET?`<span class="live ${isOnline(q.uid)?'':'off'}" title="${isOnline(q.uid)?'Dalam talian':'Luar talian'}"></span> `:''}${esc(q.name)}${NET&&q.uid===UID?' <span class="chip ok">Anda</span>':''}</span><span class="money" style="${q.cash<0?'color:var(--bad)':''}">${fmt(q.cash)}</span>
